import time #Для финального кода не необходимо, только ради информации по мере прохождения ч. 3
f = open("in.txt", "r")
...
test = [] #
n = 1 # начнём счетчик для определния того, на каком этапе находится программа во время первой части
for line in f:
    ...
    print("Строка:", n) #Печатаем этап
    n += 1 #меняем данные счетчика
...
s = 1 # начнём счетчик для определния того, на каком этапе находится программа во время второй части
for elem in ll: #Аналогично первой части, запускаем цикл for, где проходимся по каждому из элементов массива
    print("Элемент:", s) #Печатаем этап
    s += 1 #меняем данные счетчика
...
#Начало части 3.2:
i = 0
bx = 0 #Это - очередной счетчик для наблюдения за ходом действия
result32 = [] #Пустой массив-плейсхолдер для результата в данной части
while i < len(temp): #Начинаем цикл цршуд с ограничением в длину ранее указанного массива
    bx += 1 #Меняем значение счетчика
    print("Элементов:", bx) #Выдаем сообщение о прогрессе
...
for elem in result33: #Начинаем проходить по списку с результатами части 3 поэлементно.
    ...
    for ele in string: #Проходим по каждому символу в строке, используя цикл for
